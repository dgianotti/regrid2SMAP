function calculate_GPM_to_SMAP36km_regridding_matrix()
...
% against a vector of GPM data to get SMAP-gridded data (i.e.,
% GPM_on_SMAP_grid = GPM_to_SMAP_mat * GPM_data(:) )

% GPM is on a 0.1 deg lat x 0.1 deg lon grid, with corners at -180 lon,
% 180 lon, -90 lat, and 90 lat.
GPM_centers_lat = (-89.95:0.1:89.95)';
GPM_centers_lon = -179.95:0.1:179.95;
GPM_centers_lat = repmat(GPM_centers_lat,1,length(GPM_centers_lon));
GPM_centers_lon = repmat(GPM_centers_lon,size(GPM_centers_lat,1),1);

% SMAP has the lats going from highest to lowest, so we're going to flip
% the M2 data to match:
GPM_centers_lat = flipud(GPM_centers_lat);
GPM_centers_lon = flipud(GPM_centers_lon);
...
max_GPM_lat_spacing = max(max(abs(diff(GPM_centers_lat,1,1)))); % Should just be 0.1 for GPM...
max_GPM_lon_spacing = max(max(abs(diff(GPM_centers_lon,1,2))));

max_SMAP_lat_spacing = max(max(abs(diff(SMAP_centers_lat,1,1))));
max_SMAP_lon_spacing = max(max(abs(diff(SMAP_centers_lon,1,2))));

r_lat = max_GPM_lat_spacing+max_SMAP_lat_spacing;
r_lon = max_GPM_lon_spacing+max_SMAP_lon_spacing;
...
N_lon_cells_added_GPM = sum(GPM_centers_lon(1,:) > (180-2*r_lon)); % We'll add this number on to both sides of the M2 lon matrices

GPM_centers_lon_extended = [GPM_centers_lon(:, (end-N_lon_cells_added_GPM+1):(end) )-360, ...
    GPM_centers_lon, ...
    GPM_centers_lon(:, 1:(N_lon_cells_added_GPM) )+360];
GPM_centers_lat_extended = [GPM_centers_lat(:, (end-N_lon_cells_added_GPM+1):(end) ), ...
    GPM_centers_lat, ...
    GPM_centers_lat(:, 1:(N_lon_cells_added_GPM) )];
...
for i = 1:size(SMAP_centers_lat,1)
    fprintf('i = %d / %d, %d percent done, estimated time remaining is %d minutes...\n',...
        i,size(SMAP_centers_lat,1),round(100*i/size(SMAP_centers_lat,1)),...
        round((toc/(i-.9999))*(size(SMAP_centers_lat,1)-i)/60));
    for j = 1:size(SMAP_centers_lat,2)
        ...
        % speedup:
        close_GPMs = abs(GPM_centers_lat_extended(:) - SMAP_centers_lat(i,j)) < 1.05*r_lat ...
            & abs(GPM_centers_lon_extended(:) - SMAP_centers_lon(i,j)) < 1.05*r_lon;
        
        % Make a matrix out of all of the close_GPMs:
        GPM_tmp_lat = [GPM_centers_lat_extended(close_GPMs)+.05, GPM_centers_lat_extended(close_GPMs)+.05, ...
            GPM_centers_lat_extended(close_GPMs)-.05, GPM_centers_lat_extended(close_GPMs)-.05]'; % 4xN
        GPM_tmp_lon = [GPM_centers_lon_extended(close_GPMs)-.05, GPM_centers_lon_extended(close_GPMs)+.05, ...
            GPM_centers_lon_extended(close_GPMs)+.05, GPM_centers_lon_extended(close_GPMs)-.05]'; % 4xN
        
        
        
        GPM_indices = find(close_GPMs);
        % For some really annoying reason, polybool won't let me do this
        % all at once, so another for loop it is:
        N = sum(close_GPMs);
        
        [i_vec,j_vec] = ind2sub(size(GPM_centers_lat_extended),GPM_indices);
...
% and numel(GPM) columns.
% As a first step, we need to turn the j_vec values (column 2) in all of
% the C{:} matrices into the proper values for the GPM_centers matrices --
% right now, they are for the GPM_extended matrices:

N = numel(C);
[nrowGPM, ncolGPM] = size(GPM_centers_lat);

n_sparse_elements = 0; % We're going to use this value to initialize the vectors of values going into the sparse matrix

% The GPM_extended matrices have 2*N_lon_cells_added_GPM + ncolGPM columns
...
for n = 1:N
    ...
    % add-on wing:
    % Case 1: Middle:
    mids = (j_ext > N_lon_cells_added_GPM) & (j_ext <= N_lon_cells_added_GPM+ncolGPM);
    j_orig(mids) = j_ext(mids) - N_lon_cells_added_GPM;
    
    % Case 2: Left wing:
    lefts = (j_ext <= N_lon_cells_added_GPM);
    j_orig(lefts) = j_ext(lefts) - N_lon_cells_added_GPM + ncolGPM;
    
    % Case 3: Right wing:
    rights = (j_ext > N_lon_cells_added_GPM+ncolGPM);
    j_orig(rights) = j_ext(rights) - N_lon_cells_added_GPM - ncolGPM;
    
    indxs = sub2ind( [nrowGPM, ncolGPM], C{n}(:,1) + 0, j_orig );
    C{n} = [indxs, C{n}(:,3) ]; % Now just 2 columns, and the indxs vectors correspond to the indices of the full GPM matrix
    n_sparse_elements = n_sparse_elements + length(indxs);
end

% So now we have a cell array of two-column matrices, where the first
% column is the index of the GPM matrix (which will correspond to a column
...
numel_SMAP = N;
numel_GPM = (nrowGPM+2)*ncolGPM;

fprintf('Creating sparse matrix...\n');
GPM_to_SMAP = sparse(i_sparse,j_sparse,v_sparse,numel_SMAP,numel_GPM);

save('GPM_to_SMAP.mat','GPM_to_SMAP');
fprintf('GPM_to_SMAP matrix saved in GPM_to_SMAP.mat\n');
